...
- Отображаемые блоки:
  ...
  - Заголовок, зарплата (min/max, валюта, период/тип через справочник) и строка «Выплаты» выводятся в одном предложении; значения `salary_frequency` поддерживают и RU, и EN варианты. Далее следуют чипы форматов работы и опыт.
...
- SEO-разметка: JSON-LD `JobPosting` формируется на основе ответа API и встраивается `<script type="application/ld+json">`. В объект включаются `title`, `description`, `hiringOrganization.name`, `baseSalary`, `jobLocation` (только `streetAddress`, `addressLocality`, `addressCountry`). Код страны фиксирован как `"RU"`. При наличии формата работы с ID `1` дополнительно указывается `jobLocationType: "TELECOMMUTE"` и текст «Вакансия, связанная с работой из дома», чтобы отразить удалённый формат. Поля `identifier`, `hiringOrganization.sameAs` и `hiringOrganization.logo` исключены по требованию SEO. Значение `employmentType` вычисляется по справочнику `app/config/employment_types_202505222228.json` и маппится на `FULL_TIME`, `PART_TIME`, `CONTRACTOR`, `INTERN`, `VOLUNTEER`.
...
4. Результаты отображаются в виде списка с пагинацией
   - Страница `/vakansii/[slug]` (SSR) использует серверную пагинацию (`page`, `limit`) и компонент `Pagination`.
...
Неразобранные (not_processed)
    ↓
[Просмотрено] → Подумать (in_progress)
    ↓
Пригласить ▾
    ├─→ 📞 Телефонное интервью ▾
    │       ├─→ 📋 Оценка ▾
...
**Особенности системы:**
- Гибкие переходы между промежуточными статусами (`phone_interview`, `interview`, `evaluation`)
- Кнопка "Просмотрено" доступна только для статуса `not_processed`
- Статусы с шевроном (▾) открывают выпадающие меню для перехода на следующий этап
