...
export default function EmployerVacanciesPageClient() {
  ...
  const filtered = useMemo(() => {
    ...
    return base.filter((v) => (v.title || '').toLowerCase().includes(q) || (v.description || '').toLowerCase().includes(q));
  }, [items, query]);
  ...
  async function load() {
    ...
    try {
      ...
      const rows: JobPosting[] = Array.isArray(payload) ? payload : payload?.items || payload?.data || payload?.job_postings || [];
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-8 py-6">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        />
        <button onClick={load} className="px-3 py-2 text-sm rounded-lg border border-gray-200 hover:bg-gray-50">
          Обновить
        </button>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Вы уверены, что хотите удалить вакансию "{vacancyToDelete.title}"?
            </p>
            <div className="flex items-center justify-end gap-2">
