/**
 * @file: app/vacancy/[slug]/page.tsx
 * @description: Страница списка вакансий по городу (SSR). Загружает данные по city_slug и рендерит карточки вакансий.
...
export default async function VacancyBySlugPage({ params }: { params: { slug: string } }) {
  const { slug } = params
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_PATH || ''}/api/jobs/searchBySlug?city_slug=${encodeURIComponent(slug)}`, {
    // Позволяет ISR/кешировать при необходимости; сейчас всегда актуальные данные
    cache: 'no-store',
  })
  if (!res.ok) {
    return (
